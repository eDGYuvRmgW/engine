...
from flaris.rendering.mesh import Mesh
from flaris.rendering.shader import Shader

from flaris.rendering.lighting.light import Light
...
class MeshRenderer:  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, camera: Camera, shader: Shader = DEFAULT_MESH_SHADER):
        ...
        vertices = np.array([
            ...
            -0.5, 1.0, 1.0, 0.5, 0.5, -0.5, 1.0, 1.0, -0.5, 0.5, -0.5, 0.0, 1.0,
            -0.5, -0.5, -0.5, 0.0, 0.0, -0.5, -0.5, 0.5, 0.0, 0.0, 0.5, -0.5,
            0.5, 1.0, 0.0, 0.5, 0.5, 0.5, 1.0, 1.0, 0.5, 0.5, 0.5, 1.0, 1.0,
            -0.5, 0.5, 0.5, 0.0, 1.0, -0.5, -0.5, 0.5, 0.0, 0.0, -0.5, 0.5, 0.5,
            1.0, 0.0, -0.5, 0.5, -0.5, 1.0, 1.0, -0.5, -0.5, -0.5, 0.0, 1.0,
            -0.5, -0.5, -0.5, 0.0, 1.0, -0.5, -0.5, 0.5, 0.0, 0.0, -0.5, 0.5,
            0.5, 1.0, 0.0, 0.5, 0.5, 0.5, 1.0, 0.0, 0.5, 0.5, -0.5, 1.0, 1.0,
            0.5, -0.5, -0.5, 0.0, 1.0, 0.5, -0.5, -0.5, 0.0, 1.0, 0.5, -0.5,
            0.5, 0.0, 0.0, 0.5, 0.5, 0.5, 1.0, 0.0, -0.5, -0.5, -0.5, 0.0, 1.0,
            0.5, -0.5, -0.5, 1.0, 1.0, 0.5, -0.5, 0.5, 1.0, 0.0, 0.5, -0.5, 0.5,
            1.0, 0.0, -0.5, -0.5, 0.5, 0.0, 0.0, -0.5, -0.5, -0.5, 0.0, 1.0,
            -0.5, 0.5, -0.5, 0.0, 1.0, 0.5, 0.5, -0.5, 1.0, 1.0, 0.5, 0.5, 0.5,
            1.0, 0.0, 0.5, 0.5, 0.5, 1.0, 0.0, -0.5, 0.5, 0.5, 0.0, 0.0, -0.5,
            0.5, -0.5, 0.0, 1.0
        ],
                            dtype=np.float32)
    ...
    def draw(self, light: Light, mesh: Mesh, # pylint: disable=unused-argument
        ...
        gl.glUniform3f(
            ...
            light.ambient.x, light.ambient.x, light.ambient.x)
        gl.glUniform3f(
            gl.glGetUniformLocation(self.shader.program, "light.diffuse"),
            light.diffuse.x, light.diffuse.x, light.diffuse.x)
        gl.glUniform3f(
...
class Joint(): # pylint: disable=too-few-public-methods
    ...
    def __init__(self, index: int, name: str, local_bind_transform: glm.mat4):
        """Do something."""
        self.index = index
        self.name = name
        ...
        self.children = []
        self._animated_transform = None
        self._inverse_bind_transform = None

    def add_child(self, child: Joint) -> None:
        """Do something."""
        self.children.append(child)

    def _calculate_inverse(self, parent_bind_transform: glm.mat4) -> None:
        """Do something."""
        self.inverse_bind_transform = glm.inverse(parent_bind_transform *
                                                  self.local_bind_transform)

        for child in self.children:
            self._calculate_inverse(child)

    @property
    def inverse_bind_transform(self) -> glm.mat4:
        """Do something."""
        return self._inverse_bind_transform

    @property
    def animated_transform(self) -> glm.mat4:
        """Do something."""
        return self._animated_transform

    @animated_transform.setter
    def animated_transform(self, animated_transform: glm.mat4) -> None:
        """Do something."""
        self._animated_transform = animated_transform
